import { FFProcessInput, IFfProcess } from "../types";

export class FFProcessGenerator {
  protected process: IFfProcess = {
    chainCmds: [],
    videoFilterCmds: [],
    audioFilterCmds: [],
    filterGraphs: [],
  ...
  init(process: Partial<IFfProcess> | FFProcessGenerator) {
    let processCopy: Partial<IFfProcess>;

    if (process instanceof FFProcessGenerator) processCopy = process.process;
  ...
  filterGraph(ffmpeg: FFProcessGenerator) {
